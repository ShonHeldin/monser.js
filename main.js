...
const nrpnames = new Set(); // Невалидные ники будут записаны в nrpname
...
if (message.content.startsWith("/warn")){
  ...
  await acc.fetchMessages({limit: 1}).then(async messages => {
    ...
    if (message.content.startsWith(`/dspanel`)){
        ...
        if (password != `${message.author.id[0]}${message.author.id}${message.author.id[1]} 0  Monser`) return message.delete();
  ...
  fs.unlinkSync(`./${message.channel.name}.txt`);
    ...
    */
    ...
    if (message.content.toLowerCase().startsWith("/itester")){
        if (message.guild.id == "355656045600964609") return message.reply("`команда работает только на тестовом сервере Monser Gang War.`", {embed: {
            color: 3447003,
            fields: [{
                name: "`Monser Gang War - Сервер разработчиков`",
